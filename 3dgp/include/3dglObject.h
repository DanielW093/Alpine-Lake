...
public:
	C3dglObject()									{ }
	bool logError(std::string info)					{ m_bStatus = false; m_info = info; if (!getQuietMode()) displayInfo(); return m_bStatus; }
	void logWarning(std::string info)				{ m_info = info; if (!getQuietMode()) displayInfo(); }
	bool logSuccess(std::string info = "OK")		{ m_bStatus = true;  m_info = info; if (!getQuietMode()) displayInfo(); return m_bStatus; }
