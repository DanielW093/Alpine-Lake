...
void C3dglSkyBox::render()
{
	...
	GLuint attribVertex = pProgram->GetStdAttribLocation(C3dglProgram::ATTR_VERTEX);
	GLuint attribNormal = pProgram->GetStdAttribLocation(C3dglProgram::ATTR_NORMAL);
	GLuint attribTexCoord = pProgram->GetStdAttribLocation(C3dglProgram::ATTR_TEXCOORD);
	GLuint locationMatrixModelView = pProgram->GetStdUniformLocation(C3dglProgram::UNI_MODELVIEW);

	// send model view matrix
	float modelviewMatrix[16];
	glGetFloatv(GL_MODELVIEW_MATRIX, modelviewMatrix);
	modelviewMatrix[12] = modelviewMatrix[13] = modelviewMatrix[14] = 0;
	glUniformMatrix4fv(locationMatrixModelView, 1, false, modelviewMatrix);
